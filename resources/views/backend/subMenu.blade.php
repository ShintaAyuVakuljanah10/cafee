                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" id="icon" class="form-control" placeholder="Contoh: mdi mdi-view-dashboard">
                            </div>
    ...
    <script>
        $(function () {
            ...
            function loadRouteList() {
                // Kita ambil data routes yang sudah di-passing dari Controller via compact
                const routes = @json($routes); 
                let html = '<option value="">Pilih Route</option>';
                
                routes.forEach(route => {
                    // value tetap nama route asli (untuk database), label-nya yang bersih
                    html += `<option value="${route.cleaned}">${route.cleaned}</option>`;
                });
                $('#route').html(html);
            }
                ...
                $.get("{{ route('backend.submenu.data') }}", function (data) {
                    ...
                    data.forEach((item, i) => {
                        ...
                        subMenuTable.row.add([
                            i + 1,
                            item.name,
                            `<i class="${item.icon}"></i> <span class="ms-1">${item.icon}</span>`,
                            item.route ?? '-',
                            item.parent_name, // Sesuaikan dengan key yang dikirim Controller
                            item.active ? '<span class="badge bg-success">Yes</span>' : '<span class="badge bg-secondary">No</span>',
                            btnAksi // Masukkan variabel tombol di sini
                        ]);
                    });
            ...
            function loadParentMenu() {
                ...
                $.get("{{ route('backend.menu.parent') }}", function (data) {
                    ...
                    console.error("Gagal mengambil data parent menu. Pastikan route 'parent' mengembalikan JSON.");
                });
            }
            ...
            $(document).on('click', '.btn-edit', function () {
                ...
                $.get(`/backend/submenu/${id}`, function (data) {
                    $('.modal-title').text('Edit Data Sub Menu');
                    
                    $('#submenu_id').val(data.id);
                    $('#name').val(data.name);
                    $('#icon').val(data.icon);
                    
                    // Karena data.route di DB sudah bersih, dia akan otomatis terpilih
        ...
        $('[data-target="#modalSubMenu"]').click(function() {
            $('#formSubMenu')[0].reset();
            $('#submenu_id').val('');
